...
## Overall structure and main elements of Veg-X (ver. 2.0.0)
...
surfaceCover | TO BE DONE | Yes |
surfaceCoverObservation | TO BE DONE | Yes |
vegetationObservation | TO BE DONE | Yes |
...
communityObservationID | A reference to a specific communityObservation by ID. Note that the relationship is one-to-one. Only one communityObservation (with potentially many measurements inside) is allowed for each plotObservation.  | 0..1 | Yes |
...
A taxon name string occurring within the dataset. This is a usage concept defined by the dataset author which may follow, or be according, to an additional third party. It may be further related to a published name (taxon name) and or taxonomic name (taxon concept) by using the *assertedNameID* attribute of its sub-element *authorTaxonName* or alternatively relate the name to a published name (taxon name) and or taxonomic name (taxon concept) in *taxonDetermination*. Currently, the **VegX package** does not support taxon determinations, and only the sub-element *authorTaxonName* is available, which identifies uniquely the taxon name usage concept.

Sub-element               | Description                                        |  #  |  R  |
------------------------- | -------------------------------------------------- | --- | --- |
authorTaxonName               | A string representing a taxon name that the author of the dataset used to refer to a taxon observed within the plot.       | 1 | Yes |
authorTaxonCode               | Verbatum short code used by the author to signify the species at time of observation. | 0..1 | No |
voucher                   | Herbarium accession number for any archived voucher specimens. | 0..n | No |
partyWithRole             | The party that recorded a specific taxon occurrence event, or other parties with different roles.  | 0..n | No |
accordingTo               | A third party that had previously defined this usage concept.  | 0..1 | No |
...
An element *individualOrganism* represents an organism recorded during one or more observation events and identified through an identification label (e.g. tree tag number). In Veg-X documents, individual organisms may or may not have been given a taxon name (i.e. a link via sub-element *taxonNameUsageConceptID*), and the standard allows specifying the relative position of individuals within the plot to which they belong, as well as to specify related individuals. From the perspective of the **VegX package** user, individual organisms are identified by *plotID* (i.e. plot name) and *identificationLabel* (i.e. a tag number or string).

Sub-element               | Description                                        |  #  |  R  |
------------------------- | -------------------------------------------------- | --- | --- |
plotID                   | A reference to a specific plot by the plot ID.	 | 1 | Yes |
identificationLabel      | A label that is associated with an individual (e.g. a numerical tree tag). | 1 | Yes |
taxonNameUsageConceptID  | A reference to a specific taxonNameUsageConcept representing a taxon occurrence by ID. | 0..1 | Yes |
...
### Element *surfaceCover*

The definition of a surface cover class, not the observation of cover.  From the perspective of the **VegX package** user, surface cover classes are uniquely identified by *surfaceName*.

Sub-element (quantitative) | Description                                        |  #  |  R  |
-------------------------- | -------------------------------------------------- | --- | --- |
surfaceName                | Name associated with this surface cover class and which identifies it. | 1 | Yes |
methodID                   | A reference to a specific method used to define this surface cover class. | 0..1 | Yes |
definition                 | A longer description of the surface cover class definition. | 0..1 | Yes |


### Element *surfaceCoverObservation*

A *surfaceCoverObservation* is a single cover measurement applying to a surface cover class in a single plot during a single date-time period. From the perspective of the **VegX package** user, surface cover observations are uniquely identified by *surfaceID* (i.e. surface cover name) and *plotObservationID* (i.e. plot name and observation start date).

Sub-element (quantitative) | Description                                        |  #  |  R  |
-------------------------- | -------------------------------------------------- | --- | --- |
surfaceCoverID             | A reference to a specific surface cover class by ID. | 1 | Yes |
plotObservationID          | A reference to a specific plotObservation by ID. | 1 | Yes |
coverMeasurement           | A cover measurement, usually in percent. | 1 | Yes |
