...
The identity of an organism (or a set of organisms) occurring within the dataset. This is initially a name defined by the dataset author which may follow, or not, nomenclatural codes. The identity may be complemented with an scientific name (taxon name) accepted according to some nomenclatural authority, by using the sub-element *preferredTaxonNomenclature*. The taxonomic concept (taxon name + reference) that the author of the observation interpreted when observing the organism (or that a third party thinks he had in mind) can also be specified in the sub-element *originalConceptIdentification*. Subsequent re-evaluations of the taxon concept (e.g. after inspection of the herbarium voucher) by third parties should be specified using the main element *taxonDetermination* (which are not currently supported by the **VegX package**).
...
preferredTaxonNomenclature | The interpretation of the nomenclature that should be applied to organism identity, made after the observation event by the author of the data set or a third party. The sub-element *preferredTaxonNameID* points to an organism name that is considered the preferred name for the organism identity. | 0..1 | Partial |

In the **VegX package** a field called **organismIdentityName** is used to name organisms with the following rules:
...
Organism identities are by default kept separate when merging Veg-X documents using the VegX package. If the possibility of merging of identities is enforced by the user, the decision to actually merge two given organism identities distinguishes three cases depending on the availability of taxon concepts:

1. If none of the two organism identities has an associated taxon concept (i.e. if *originalConceptIdentification* is missing in both identities) then the two organism identities are considered the same if they have the same *organismIdentityName*. Three sub-cases are distinguished:
    i) None of the organism identities have *preferredTaxonNomenclature*, so the decision to merge depends exclusively on having the same *originalOrganismName*.
    ii) If one of the organism identities has a *preferredTaxonNomenclature* and the other does not, merging will occur if the *originalOrganismName* of the latter is equal to the *preferredTaxonName* of the former. The result of merging the elements in this case will be an organism identity where *originalOrganismName = preferredTaxonName*.
    iii) If the two identities to be merged have the same *preferredTaxonName* (and hence the same *organismIdentityName*) but different *originalOrganismName*. In that case, the merging function will again return an identity where both *originalOrganismName = preferredTaxonName*.
2. If only one of the organism identities has an associated taxon concept (i.e. if *originalConceptIdentification* is only specified for one of the organism identities) then the two identities cannot be merged. The user should in this case specify a taxon concept for the organism identity that lacks it before attempting to merge their identities.
3. If both organism identities have an associated taxon concept (i.e. if *originalConceptIdentification* is specified in both identities) then the two identities are considered the same if they refer to the same taxon concept (i.e. if both the taxon name and bibliographic reference are the same). In this case the result of the merging operation is an organism identity with: (a) *originalConceptIdentification* indicating the (common) taxon concept; (b) *originalOrganismName* equal to the taxon name that forms the taxon concept; (c) no element *preferredTaxonNomenclature*, because the two identities to be merged could differ in their preferred nomenclature. Thus, nomenclature revision may be advisable after merging identities that have taxon concepts defined.
