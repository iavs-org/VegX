...
![**Fig. 1**. Schematic diagram showing (a) problem: multiple tools and databases and (b) solution: sharing tools through a common standard.](DataExchange.png) 
...
studyAreaDescription      | Description of the physical area associated with the research project, potentially including coverage, climate, geology, disturbances, etc. | 0..1 | Yes |
...
verticalCoordinates | Elevation of the plot in respect to some vertical datum (such as the mean sea level or an ellipsoid). | 0..n | Partial |
...
interpretationPartyID     | A link to a party who undertook the nomenclature revision. | 0..n | Partial |
...
The *location* of an organism is assumed to be constant in time during the organism lifespan. Sub-elements *horizontalLocation* are used to store x-y (or polar) coordinates of organism, either absolute or in relation to the plot origin or a related organism. To avoid ambiguity there should be only one coordinate pair for an organism. However, the Veg-X schema can accommodate multiple measurements made by different parties or at different times. The R package does not yet support *location* elements for organisms.
