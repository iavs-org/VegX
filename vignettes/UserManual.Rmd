...
In this vignette you will learn how to use the **VegX R package** to map, integrate and harmonize vegetation data having the Veg-X standard as data model underlying the whole process. For the examples, we use the example data sets provided in the package. If you do not know what the Veg-X standard is, please refer to vignette **The Veg-X exchange standard**. In the following, we usually refer to elements of the Veg-X standard. Readers should refer to the same vignette to understand the definition of these elements and their logical relationships.

## Loading the package and source data
...
comb_vegx = transformQuantitativeScale(comb_vegx, "Stratum height/m", heightMethod2,
                               function(x){return(x*10)}, replaceValues = TRUE)
showElementTable(comb_vegx, "stratumObservation")
```
