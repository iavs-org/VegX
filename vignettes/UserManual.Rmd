...
By plot geometry, we refer to plot area, shape and dimensions. Veg-X allows different plot shapes, and each plot shape implies different dimensions. Plot geometry is specified using function ``addPlotGeometries()``, which like ``addPlotLocation()`` will replace any previous information. We start by looking at the plot geometry data in the Mokihinui data set:
```{r inspect plot geom moki}
names(moki_site)
table(moki_site$Shape)
```
which leads us to define the following mapping for rectangular (or square) plots:
