...
```
Note that the column ``individualOrganismLabel`` contains labels created by the function itself. The Mt Fyffe forest data set also contains tree diameter measurements, but in this case there have been two surveys, so we need the mapping to specify which column identifies each tree:
